...
- **활성화** (기본): 아래 키들을 가로채서 내비게이션에 사용합니다. IDA의 단일 키 단축키(`n` 이름 변경, `d` 데이터, `u` 정의 해제, `g` 주소 이동, `c` 코드 변환, `*` 배열, `#` 숫자 변환, `m` enum 등)보다 우선합니다.
...
그 외 모든 키(`Esc`, `Enter`, `x`, `Space`, 방향키 등)는 idavim이 활성화된 상태에서도 IDA로 그대로 전달됩니다. 유일한 예외는 `f`/`F`/`g`/`c`/`m`/`` ` `` 직후의 키 하나입니다: 인쇄 가능한 키는 해당 명령을 완성하고, 일반 `Esc`는 IDA의 "뒤로 가기"를 발동시키지 않고 명령만 취소합니다 (vim과 동일).
...
- **미지원**: `카운트 + f/F` (`3fx`는 1번째 매치로만 감 — `f` 후 `;;`를 쓰세요), 오퍼레이터·텍스트 오브젝트 (`dw`, `yy`, `ci(` 등), 레지스터, 매크로
