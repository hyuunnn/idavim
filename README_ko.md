...
- **활성화** (기본): 아래 키들을 가로채서 내비게이션에 사용합니다. IDA의 단일 키 단축키(`n` 이름 변경, `d` 데이터, `u` 정의 해제, `g` 주소 이동, `c` 코드 변환, `*` 배열, `#` 숫자 변환, `m` enum, `y` 타입 설정 등)보다 우선합니다.
...
마크는 IDA 북마크로 저장되므로 데이터베이스에 남고 IDA의 Bookmarks 위젯에도 표시됩니다(`idavim: a` 형식) — 위젯에서 지우면 마크도 지워집니다. 마크는 줄·컬럼이 아니라 주소를 기억하기 때문에(브레이크포인트처럼), 어느 뷰에서 점프하든 현재 뷰에서 그 주소가 표시된 줄로 이동합니다.
...
| `yy` | 현재 줄의 주소를 클립보드에 복사 (예: `0x401000`) |

그 외 모든 키(`Esc`, `Enter`, `x`, `Space`, 방향키 등)는 idavim이 활성화된 상태에서도 IDA로 그대로 전달됩니다. 유일한 예외는 `f`/`F`/`g`/`c`/`m`/`` ` ``/`y` 직후의 키 하나입니다: 인쇄 가능한 키는 해당 명령을 완성하고, 일반 `Esc`는 IDA의 "뒤로 가기"를 발동시키지 않고 명령만 취소합니다 (vim과 동일).
...
- **미지원**: `카운트 + f/F` (`3fx`는 1번째 매치로만 감 — `f` 후 `;;`를 쓰세요), 오퍼레이터·텍스트 오브젝트 (`dw`, `ci(` 등), 레지스터, 매크로
