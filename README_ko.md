...
- **활성화** (기본): 아래 키들을 가로채서 내비게이션에 사용합니다. IDA의 단일 키 단축키(`n` 이름 변경, `d` 데이터, `u` 정의 해제, `g` 주소 이동 등)보다 우선합니다.
...
그 외 모든 키(`Esc`, `Enter`, `x`, `Space`, 방향키 등)는 idavim이 활성화된 상태에서도 IDA로 그대로 전달됩니다.
